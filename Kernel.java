/*
 * $Log: Kernel.java,v $
 * Revision 1.6  2001/08/19 23:20:00  rayo
 * fixed runnable
 *
 * Revision 1.5  2001/07/30 03:49:16  rayo
 * changed E to H
 *
 * Revision 1.4  2001/07/30 02:37:29  rayo
 * increased the sleep time from 100 to 1000 msec
 *
 * Revision 1.3  2001/07/30 01:14:20  rayo
 * removed 100 and 1000 which were added to resource and process ids
 *
 */
...
public void run() {
    ...
    synchronized (this) {
        try {
            ...
            System.out.println("Initial wait interrupted, continuing execution...");
    ...
    while (true) {
        ...
        if (stepping) {
            synchronized (this) {
                try {
                    ...
                    System.out.println("Step wait interrupted, resuming...");
                    Thread.currentThread().interrupt();
                }
            }
        } else {
        ...
        if (processCount == haltedCount || processCount == blockedCount) {
            ...
            synchronized (this) {
                try {
                    ...
                    System.out.println("Final wait interrupted, resuming...");
                    Thread.currentThread().interrupt();
                }
            }
        }
